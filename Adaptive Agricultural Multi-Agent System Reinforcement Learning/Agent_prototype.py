...
def get_tension(civ):
    ...
    reserve_days = civ.calories / daily_need
...
def get_variety_memory(civ,varietyID):
...
def update_max_amount_by_variety(civ,food,variety_memory):
    total_calories=get_total_food_value(food)
    if total_calories>variety_memory["best_calories_amount"]:
        variety_memory["best_height"]=food[0].parentSeed.originalHeight
...
def cultivate(civ, board, heightMetrics):
    ...
    for s in civ.seeds:
        ...
        variety_memory=get_variety_memory(civ,s.parentVariety.varietyID)
        update_max_amount_by_variety(civ,food,variety_memory)
        if s.parentVariety.varietyID not in varieties:
            varieties.append(s.parentVariety)
    civ.seeds=crop_seeds
    civ.food=crop_food
...
def execute_action(civ, action, heightMetrics, board):
    new_pos=civ.position
    if action == "cultivate":
        reward = cultivate(civ, board, heightMetrics)
    else:
...
def main():
    ...
    for i in range(1000):
        ...
        next_state,reward=execute_action(civ, action,heightMetrics, board)
        ...
        civ.tension=get_tension(civ)
        increase=0
        if (civ.tension<0.75 and civ.calories>3000+civ.population*3000)or civ.calories>civ.population*5000 and action=="cultivate":
